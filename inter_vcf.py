#/usr/lib/python
from HTSeq import VCF_Reader
from optparse import OptionParser

msg_usage = 'usage: %prog [-F] fb_file(nofile) [-G] gatk_file(nofile) \
[-S] sb_file(nofile)'
descr ='''get the common snp site from three different softwares.
generate three files contain common sites with three softwares info
'''
optparser = OptionParser(usage = msg_usage, description = descr)
optparser.add_option('-F', '--fb', dest = 'fbfile', default='nofile',
                     help = 'Input the vcf file generated by freebays.')
optparser.add_option('-G', '--gatk', dest = 'gatkfile',default='nofile',
                     help = 'Input the vcf file generated by GATK.')
optparser.add_option('-S', '--sb', dest = 'sbfile',default='nofile',
                     help = 'Input the vcf file generated by samtools.')
options, args = optparser.parse_args()

def common3_vcf(x, y, z):
    '''find the common site in those three files.
    write the common sites to a new file with
    their information.
    '''
    vf1 = VCF_Reader(x)
    vf2 = VCF_Reader(y)
    vf3 = VCF_Reader(z)
    vf1_set = set()
    vf2_set = set()
    vf3_set = set()
    for i in vf1:
        vf1_set.add(i.chrom+'-'+str(i.pos.pos))
    for i in vf2:
        vf2_set.add(i.chrom+'-'+str(i.pos.pos))
    for i in vf3:
        vf3_set.add(i.chrom+'-'+str(i.pos.pos))
    common_set = vf1_set & vf2_set & vf3_set
    cn1 = open('common_sites_fb.vcf', 'w')
    for i in open(x, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn1.write(i)
    cn1.close()

    cn2 = open('common_sites_gatk.vcf', 'w')
    for i in open(y, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn2.write(i)
    cn2.close()

    cn3 = open('common_sites_sb.vcf', 'w')
    for i in open(z, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn3.write(i)
    cn3.close()

def commonFG_vcf(x, y):
    '''find the common site in freebayse and gatk.
    write the common sites to a new file with their info.
    '''
    vf1 = VCF_Reader(x)
    vf2 = VCF_Reader(y)
    vf1_set = set()
    vf2_set = set()
    for i in vf1:
        vf1_set.add(i.chrom+'-'+str(i.pos.pos))
    for i in vf2:
        vf2_set.add(i.chrom+'-'+str(i.pos.pos))
    common_set = vf1_set & vf2_set
    cn1 = open('common_sites_fb.vcf', 'w')
    for i in open(x, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn1.write(i)
    cn1.close()

    cn2 = open('common_sites_gatk.vcf', 'w')
    for i in open(y, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn2.write(i)
    cn2.close()

def commonFS_vcf(x, y):
    '''find the common site in freebayse and samtolls files.
    write the common sites to a new file with
    their information.
    '''
    vf1 = VCF_Reader(x)
    vf2 = VCF_Reader(y)
    vf1_set = set()
    vf2_set = set()
    for i in vf1:
        vf1_set.add(i.chrom+'-'+str(i.pos.pos))
    for i in vf2:
        vf2_set.add(i.chrom+'-'+str(i.pos.pos))
    common_set = vf1_set & vf2_set
    cn1 = open('common_sites_fb.vcf', 'w')
    for i in open(x, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn1.write(i)
    cn1.close()

    cn2 = open('common_sites_sb.vcf', 'w')
    for i in open(y, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn2.write(i)
    cn2.close()

def commonGS_vcf(x, y):
    '''find the common site in gatk and samtools.
    write the common sites to a new file with
    their information.
    '''
    vf1 = VCF_Reader(x)
    vf2 = VCF_Reader(y)
    vf1_set = set()
    vf2_set = set()
    for i in vf1:
        vf1_set.add(i.chrom+'-'+str(i.pos.pos))
    for i in vf2:
        vf2_set.add(i.chrom+'-'+str(i.pos.pos))
    common_set = vf1_set & vf2_set

    cn1 = open('common_sites_gatk.vcf', 'w')
    for i in open(x, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn1.write(i)
    cn1.close()

    cn2 = open('common_sites_sb.vcf', 'w')
    for i in open(y, 'r'):
        if not i.startswith('#'):
            obj = i.split()[0] + '-' + i.split()[1]
            if obj in common_set:
                cn2.write(i)
    cn2.close()

if __name__ == '__main__':
    fb = options.fbfile
    gatk = options.gatkfile
    sb = options.sbfile
    if fb == 'nofile':
        commonGS_vcf(gatk, sb)
    elif gatk == 'nofile':
        commonFS_vcf(fb, sb)
    elif sb == 'nofile':
        commonFG_vcf(fb, gatk)
    else:
        common3_vcf(fb, gatk, sb)

